...
if [ $ACTION == "swift-package" ]; then
  ...
  if [ -n "$DESTINATION" ]; then
    xcodebuild -project generated/CacheAdvance.xcodeproj -scheme "CacheAdvance-Package" -sdk $SDK -destination "$DESTINATION" -configuration Release -PBXBuildsContinueAfterErrors=0 build
  else
